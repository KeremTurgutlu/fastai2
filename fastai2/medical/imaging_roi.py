...
__all__ = ['dicom_tags', 'dicom_modality', 'modality', 'NotStructTypeException', 'NotImageTypeException', 'spacings',
           'roi_names', 'poly2mask']
...
#Cell
class NotImageTypeException(Exception):
    def __init__(self, modality): self.modality = modality
    def __str__(self): return f"This method or property is only available for Image modality types, such as CT or MR, not for {self.modality}"

#Cell
@patch_property
def spacings(self:DcmDataset):
    'x,y,z spacings in mm'
    if self.modality in (dicom_modality.ct, dicom_modality.mr):
        return array(list(self[dicom_tags.pixel_spacing].value) +
              [self[dicom_tags.slice_thickness].value], dtype=np.float32)
    else: raise NotImageTypeException(self.modality)
